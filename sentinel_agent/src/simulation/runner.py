...
from src.protocol.channel import InMemoryChannel, MessageLog
...
async def _run_three_satellite_simulation(
    ...
    def build_pair(alert_init, alert_resp, init_id, resp_id, send_ch, recv_ch):
        init_graph = build_initiator_graph(llm=llm, send_channel=send_ch, receive_channel=recv_ch)
        resp_graph = build_responder_graph(llm=llm, send_channel=recv_ch)
    ...
    pair_ab = build_pair(alert_ab, alert_ba, sat_a, sat_b, a_to_b, b_to_a)
    pair_ac = build_pair(alert_ac, alert_ca, sat_a, sat_c, a_to_c, c_to_a)
    pair_bc = build_pair(alert_bc, alert_cb, sat_b, sat_c, b_to_c, c_to_b)
...
async def run_simulation(
    ...
    if scenario == "three_way":
        ...
        decisions, result = await _run_three_satellite_simulation(llm, message_logs)
        ...
        decision, result = await _run_two_satellite_simulation(scenario, llm, message_logs)
